...
public class CondominioService {
	...
	public Condominio atualizar(Long id, Condominio condominio) {
		...
		Endereco endereco = enderecoRepository.findById(condominio.getEndereco().getId()).orElse(null);
		if (endereco == null) {
	...
	public Condominio salvar(@Valid Condominio condominio) {
		// Verificar se o endereço(id) existe
		Endereco endereco = enderecoRepository.findById(condominio.getEndereco().getId())
				.orElseThrow(() -> new EnderecoInexistenteException());
		condominio.setEndereco(endereco);
